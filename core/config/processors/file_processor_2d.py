...
from core.config.parsers.data_2d_parser import DataParser2D
from core.config.readers.file_reader2d import FileReader2D
import pandas as pd
...
class ConfigurationFileProcessor2D(IConfigurationFileProcessor):
    ...
    def process(self):
        ...
        self.data[['X', 'Y']] = apply_lattice_vectors(
            self.data[['X', 'Y']].values,
            self.vectors,
        )
        self.data[['Xav', 'Yav']] = apply_lattice_vectors(
            self.data[['Xav', 'Yav']].values,
            self.vectors,
        )
