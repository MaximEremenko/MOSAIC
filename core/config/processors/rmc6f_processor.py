...
from core.config.metadata.rmc6f_metadata_extractor import (
    RMC6fMetadataExtractor,
)
from core.config.parsers.rmc6f_data_parser import RMC6fDataParser
from core.config.readers.rmc6f_file_reader import RMC6fFileReader
from core.structure import CellCalculator, apply_lattice_vectors
import pandas as pd
import numpy as np
...
class RMC6fProcessor( ):
    ...
    def process(self):
        ...
        self.original_coordinates = pd.DataFrame(
            apply_lattice_vectors(self.data_frame[['x', 'y', 'z']].to_numpy(dtype=float), self.vectors),
            columns=self.data_frame[['x', 'y', 'z']].columns,
        )
        ...
        self.average_coordinates = pd.DataFrame(
            apply_lattice_vectors(self.data_frame[['x', 'y', 'z']].to_numpy(dtype=float), self.vectors),
            columns=self.data_frame[['x', 'y', 'z']].columns,
        )
    ...
    def get_cells_origin (self) -> pd.DataFrame:
        return pd.DataFrame(
            apply_lattice_vectors(
                (self.cell_ids / self.supercell).to_numpy(dtype=float),
                self.vectors,
            ),
            columns=['x', 'y', 'z'],
        )
