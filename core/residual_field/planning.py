...
def estimate_chunk_partition_bytes(point_rows: list[dict[str, object]]) -> tuple[int, int, int]:
    if not point_rows:
        return 1, 0, 0
    dimensionality = int(len(np.asarray(point_rows[0]["coordinates"]).reshape(-1)))
    estimated_rifft_points = sum(
...
def partition_residual_field_work_units(
    ...
    *,
    ...
    for work_unit in work_units:
        ...
        for partition_id, selection in enumerate(np.array_split(point_indices, target_partitions)):
            if selection.size == 0:
                continue
            partitioned.append(
