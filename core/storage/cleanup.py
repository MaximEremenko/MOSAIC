...
from core.storage.publisher import public_manifest_path, validate_public_manifest_files
...
def _cleanup_scattering_chunk(
    *,
    ...
    payload_hashes = {candidate.payload_sha256 for candidate in valid_candidates}
    if len(payload_hashes) != 1:
        for candidate in valid_candidates:
            retained.append(str(candidate.candidate_id))
        skipped.append(f"scattering chunk {chunk_id}: retained conflicting candidates")
    else:
        for candidate in valid_candidates:
...
def _cleanup_residual_chunk(
    *,
    ...
    if len(payload_hashes) != 1:
        for candidate in valid_candidates:
            retained.append(str(candidate.candidate_id))
        skipped.append(f"residual chunk {chunk_id}: retained conflicting candidates")
    else:
        for candidate in valid_candidates:
            if candidate.candidate_id == selected.candidate_id:
                continue
