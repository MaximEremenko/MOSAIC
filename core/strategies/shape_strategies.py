...
# ----------------------------------------------------------------------
#  GPU device helpers – same mathematics, CUDA-safe
# ----------------------------------------------------------------------
MAX_SOL = 256*16                      # safe upper bound (adapt if needed)

import numpy as np
import math
from numba import cuda, float32, int32, boolean, njit
...
def find_val_in_interval_dev(cmin, cmax, a, tol, out):
    """
    GPU/device version.
    *out* is a pre-allocated cuda.local.array(MAX_SOL).
    Returns number of solutions written.
    """
    ...
    # simple insertion sort + rounding + uniq in place
    for i in range(cnt):
        # round
        out[i] = round8(out[i])
        # insertion‐sort the current element
        j = i
    ...
    if cnt > 0:
        ...
        for i in range(1, cnt):
            if out[i] != last:
                ...
                last = out[i]
                k += 1
    return k  # final count after uniq


# ----------------------------------------------------------------------
#  GPU kernel – one thread per point
# ----------------------------------------------------------------------
@cuda.jit
def mask_kernel(points, radii, centers,
                cmin, cmax, mask):
    i = cuda.grid(1)
    if i >= points.shape[0]:
        return
    ...
    # local scratch arrays for interval solutions
    ...
    for m in range(M):
        ...
        r2 = r * r
        # nested loops over all images of this circle
        inside = False
        ...
        if inside:
            mask[i] = True
            return    # no need to inspect other circles


# ----------------------------------------------------------------------
#  Host helper: convenient wrapper around the kernel call
# ----------------------------------------------------------------------
def compute_2d_mask_gpu(points, radii, centers,
                        coord_min, coord_max,
    ...
    mask_kernel[blocks, threads_per_block](d_points,
                                           d_radii,
                                           d_centers,
                                           d_cmin,
                                           d_cmax,
