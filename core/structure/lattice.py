...
def apply_lattice_vectors(coords: np.ndarray, vectors: np.ndarray) -> np.ndarray:
    """
    Apply small lattice-vector transforms without delegating to BLAS/OpenMP.

    This keeps 1D/2D/3D coordinate transforms in the config/structure path
    deterministic in environments where tiny `dot()` / `@` calls can abort.
    """
    ...
    if vectors_arr.shape != (dim, dim):
        raise ValueError(
            f"vectors shape {vectors_arr.shape} does not match coordinate dimension {dim}."
        )

    if dim == 1:
        return coords_arr * float(vectors_arr[0, 0])
    ...
    if dim == 2:
        ...
        return np.column_stack(
            [
                x * vectors_arr[0, 0] + y * vectors_arr[1, 0],
                x * vectors_arr[0, 1] + y * vectors_arr[1, 1],
            ]
        )
    ...
    if dim == 3:
        ...
        return np.column_stack(
            [
                x * vectors_arr[0, 0] + y * vectors_arr[1, 0] + z * vectors_arr[2, 0],
                x * vectors_arr[0, 1] + y * vectors_arr[1, 1] + z * vectors_arr[2, 1],
                x * vectors_arr[0, 2] + y * vectors_arr[1, 2] + z * vectors_arr[2, 2],
            ]
        )
